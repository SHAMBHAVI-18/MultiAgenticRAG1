...
class AuthenticationSystem:
    def __init__(self, credentials_df: pd.DataFrame):
        # Force string conversion for credentials to avoid float errors
        self.credentials = {
            str(row['dummy_email']).strip(): {
                'password': str(row['dummy_password']).strip(), 
...
class RAGOrchestrator:
    ...
    def process_query(self, query, session_id="default"):
        ...
        elif classification.intent == IntentCategory.SENSITIVE_INTERNAL: agent = self.hr_agent 
        ...
        Context: {context}
        User Question: {query}
        Instruction: Answer based on context. Be professional.
        Answer:
        """
        try:
...
def initialize_system():
    try:
        ...
        # --- THE FIX IS HERE ---
        # 1. Fill NA values with empty string
        # 2. Force convert the entire dataframe to string (astype(str))
        # 3. Join them
        texts = data_df.fillna('').astype(str).agg(' '.join, axis=1).tolist()
        
        vector_store = FAISS.from_texts(texts, embeddings)
        return data_df, creds_df, vector_store
    except Exception as e:
        # Re-raise exception so Streamlit sees it
        raise e
